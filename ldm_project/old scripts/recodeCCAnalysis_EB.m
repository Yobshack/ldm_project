...
toc
%% Group by neurons
correctedMat = cell(length(unique(geno)),3);
...
for ii = 1:size(correctedMat,1)
    ...
    for jj = 1:2
        ...
        [a,b,c] = unique(splitName);
                correctedMat{ii,jj} = correctedLDM(strcmp(geno,uniqueGeno(ii)) &...
                    strcmp(effector,effectorTypes(jj)));
                correctedMatE{ii,jj} = correctedLDMErr(strcmp(geno,uniqueGeno(ii)) &...
                    strcmp(effector,effectorTypes(jj)));
