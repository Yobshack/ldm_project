%% Select files to analyze 15 min data
...
geno = {'Cry','PanR7','GMR','Rh1','Rh1','GMR','PanR7','Cry','Rh5','PanR8','GMR','GMR','PanR8',...
    'Rh5','Rh5','PanR8','Rh4','Rh3','Rh3','Rh4','PanR8','Rh5','Rh4','Cry','PanR7',...
    'Rh1','Rh3','Rh3','Rh1'};

correctedMat = cell(length(unique(geno)),2);
...
offsets = [-0.25 0 0.25];
hold on
uniqueGeno = unique(geno);



color = colormap(gray(16));
for ii = 1:size(correctedMat,1)
    clear data error sl sd sle sde
    for jj = 1:2
        index = strcmp(geno,uniqueGeno(ii)) &...
            strcmp(effector,effectorTypes(jj));
        splitName = geno(index);
        [a,b,c] = unique(splitName);
                correctedMat{ii,jj} = correctedLDM(index);
                correctedMatE{ii,jj} = correctedLDMErr(index);
...
set(gca,'XTickLabel',labs,'XTickLabelRotation',45,'XTick',1:length(uniqueGeno),...
