...
%%
importAnno = importfile10('Brp_Data.tsv');

importAnno([139, 147, 152, 173],:) = []%importAnno([140, 148, 153, 170, 172],[10 9]) 
%importAnno([107,152],:) = [];
importAnno1 = importAnno(121:end,:);
geno = 'SS02252';
...
for ii = 1:size(importAnno,1)
    ...
    annoDayMonth(ii,:) = [month(importAnno{ii,3}), day(importAnno{ii,3}), importAnno{ii,1},...
...
opDat = cell2mat((importAnno2(:,9:10)));
% 
% flipV = [9 13 16 24 26]
...
annoDateTemp = cellfun(@(x) strsplit(x,'_'),allMat(:,4),'un',0);
...
[a,b,c] = intersect(annoDayMonth(idxThresh,1:3),annoDayMonth2,'rows');

allMat = allMat(c,:);

ldm = corrMat1(b,14); % this can be changed to analyze different behavioral metrics
 
compVect = -200:300;
allAnimalsMatrix = nan(size(allMat,1),length(compVect),3);
...
for ii = 1:size(allMat,1)
    ...
    idx2 = (1:length(y2)) - i;
%     
%     [xcf,lags,~] = crosscorr(y1,y2,100);
%     [m,i] = max(xcf);
%     idx2 = idx2 + lags(i);
    
    wholeVect1 = zeros(length(compVect),1);
    [a,b,c] = intersect(idx1,compVect);
    wholeVect1(c) = smooth(d1,10);
    
    wholeVect2 = zeros(length(compVect),1);
    [a,b,c] = intersect(idx2,compVect);
    wholeVect2(c) = smooth(d2,10);
    
    data = log2((wholeVect1+1)./(wholeVect2+1)); % this is the ratiometric comparison
    data = wholeVect1 - wholeVect2; % this is subtractive instead
    
    allAnimalsMatrix(ii,:,:) = cat(3,wholeVect1',wholeVect2',data');
end
%% Generate spatial distribution figure
figure
corPts = corr(ldm,allAnimalsMatrix(:,:,3),'rows','pairwise','type',...
    'Pearson');

varCor = bootstrp(1000,@(x) corr(ldm(x),allAnimalsMatrix(x,:,3),'rows','pairwise','type',...
    'Pearson'),1:length(ldm));
yyaxis left
set(gca,'YColor','black','XLim',[-25 75])
dat1 = allAnimalsMatrix(:,:,1);
dat2 = allAnimalsMatrix(:,:,2);
dat3 = allAnimalsMatrix(:,:,3);

plot(compVect,dat1,'LineWidth',1,'color','black','LineStyle','-','Marker','none')
hold on
...
plot(compVect,dat2,'LineWidth',1,'color','red','LineStyle','-','Marker','none')
...
ylabel('Cross Sectional Area of Average Neuron (pixels)')

yyaxis right
set(gca,'YColor','blue','XLim',[-25 75])

idx = ~isnan(corPts);
corPts = corPts(idx);
xDat = compVect(idx);
smoothNum = 1;
yDat1 = smooth(corPts,smoothNum);
yDat2 = smooth(fliplr(corPts),smoothNum);
yDat3 = smooth(nanstd(varCor(:,idx)),smoothNum);
yDat4 = smooth(fliplr(nanstd(varCor(:,idx))),smoothNum);
plot(xDat,yDat1,'color','blue','LineStyle','-','LineWidth',3)
patch([xDat fliplr(xDat)],...
    [yDat1+yDat3;yDat2-yDat4]',...
    'blue','FaceAlpha',0.3)
ylabel('Correlation to LDM across individuals')
shg

xlabel('Neuron Length (Centered on Dense Dorsal Projection)')
