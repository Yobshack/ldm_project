...
function hError = generateTTAPlot3(subArray,lightDat,behave,color,light,bin)
...
for jj = 1:size(subArray,1)
    ...
    y = subArray{jj,1}(ind);
    darkBias = [subArray{jj,13}(:,behave,2,1,bin) subArray{jj,14}(:,behave,2,1,bin) ];
    lightBias = [subArray{jj,13}(:,behave,1,1,bin)  subArray{jj,14}(:,behave,1,1,bin) ];
