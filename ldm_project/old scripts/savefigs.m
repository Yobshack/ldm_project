function ret = savefigs(filename)
...
for m = 1:length(hfigs)
    ...
    if strcmp(filename, '0')                        %Skip figure when user types 0
        ...
        saveas(hfigs(m), strcat(filename{:},'_',num2str(m), '.fig')) %Matlab .FIG file %Windows Enhanced Meta-File (best for powerpoints) %Standard PNG graphics file (best for web)
